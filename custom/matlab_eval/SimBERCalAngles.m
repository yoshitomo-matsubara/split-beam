function BER = SimBerBERCalAngles(matV,chSeeds,c)
...
chCodingVec = c.chCodingVec;  % Channendel coding for 4 users
numTx = c.numTx;
phi_bits        = 4;                            % number of bits for phi angel
psi_bits        = 2;                            % number of bits for psi angel
matV = permute(matV,[2,3,4,5,1]);
...
%     chanSeeds = [randi(1000) randi(2000) randi(3000) randi(4000)];
    ...
    for i = 1:numUsers
        ...
        v = permute(V{i},[2 3 1]);
        v = awgn(v, 10 * snr,'measured');
        v_com = bfCompQuant(v,phi_bits+2,psi_bits+2);
        steeringMatrix(:,bfIdx,:) = permute(bfDecom(v_com,numTx,numSTSAll(1),phi_bits+2,psi_bits+2),[3 2 1]);
    ...
    for i = 1:scNum %% num of suncarriers
        ...
        steeringMatrix(i,:,:) = h*pinv(h'*h + delta);
    end

    cfgVHTMU.SpatialMapping = 'Custom';
    cfgVHTMU.SpatialMappingMatrix = steeringMatrix; 
